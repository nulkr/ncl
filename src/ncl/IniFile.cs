// 
// Original Source is INIFile class V2.1, (C)2009-2013 S.T.A. snc ***
...
namespace ncl
{
    public class STA_INIFile
    {
        ...
        // *** Constructor ***
        public STA_INIFile(string FileName)
        {
            Initialize(FileName, false, false);
        }

        public STA_INIFile(string FileName, bool Lazy, bool AutoFlush)
        {
    ...
    public class IniFile : STA_INIFile
    {
        #region constant

        private readonly string TrueStr = "TRUE";
        private readonly string FalseStr = "FALSE";

        #endregion
        
        public IniFile(string sFileName) : base(sFileName) { }

        public string Read(string sSetionName, string sKey, string defaultValue)
        {
            return base.GetValue(sSetionName, sKey, defaultValue);
        }
        public int Read(string sSetionName, string sKey, int defaultValue)
        {
            return base.GetValue(sSetionName, sKey, defaultValue);
        }
        public double Read(string sSetionName, string sKey, double defaultValue)
        {
            return base.GetValue(sSetionName, sKey, defaultValue);
        }
        public DateTime Read(string sSetionName, string sKey, DateTime defaultValue)
        {
            return base.GetValue(sSetionName, sKey, defaultValue);
        }
        public bool Read(string sSetionName, string sKey, bool defaultValue)
        {
            string sRes;
            
            if (defaultValue)
                sRes = base.GetValue(sSetionName, sKey, TrueStr);
            else
                sRes = base.GetValue(sSetionName, sKey, FalseStr);

            sRes = sRes.ToUpper().Trim();
            return (sRes == TrueStr || sRes == "1");
        }
        public byte[] Read(string sSetionName, string sKey, byte[] defaultValue)
        {
            return base.GetValue(sSetionName, sKey, defaultValue);
        }
        public T Read<T>(string sSetionName, string sKey, object o)
        {
            return (T)Enum.Parse(typeof(T), base.GetValue(sSetionName, sKey, o.ToString()));
        }

        public void Write(string sSetionName, string sKey, string value)
        {
            base.SetValue(sSetionName, sKey, value);
        }
        public void Write(string sSetionName, string sKey, int value)
        {
            base.SetValue(sSetionName, sKey, value);
        }
        public void Write(string sSetionName, string sKey, double value)
        {
            base.SetValue(sSetionName, sKey, value);
        }
        public void Write(string sSetionName, string sKey, DateTime value)
        {
            base.SetValue(sSetionName, sKey, value);
        }
        public void Write(string sSetionName, string sKey, bool value)
        {
            if (value)
                base.SetValue(sSetionName, sKey, TrueStr);
            else
                base.SetValue(sSetionName, sKey, FalseStr);
        }
        public void Write(string sSetionName, string sKey, byte[] value)
        {
            base.SetValue(sSetionName, sKey, value);
        }
        public void Write(string sSetionName, string sKey, object o)
        {
            base.SetValue(sSetionName, sKey, o.ToString());
        }

        public void Load() { base.Refresh();  }
        public void Save() { base.Flush(); }

        public void AssignToControls(string sSection, Control ctrlParent)
        {
            foreach(Control ctrl in ctrlParent.Controls)                
            {
                if (ctrl.GetType() == typeof(ComboBox) && ctrl.Name.IndexOf("cb_") == 0)
                {
                    ComboBox c = ctrl as ComboBox;
                    c.SelectedIndex = Read(sSection, ctrl.Name.Substring(3), c.SelectedIndex);
                }
                else if (ctrl.GetType() == typeof(StateButton) && ctrl.Name.IndexOf("sbtn_") >= 0)
                {
                    StateButton c = ctrl as StateButton;
                    c.State = Read(sSection, ctrl.Name.Substring(5), c.State);
                }
                else if (ctrl.GetType() == typeof(CheckBox) && ctrl.Name.IndexOf("chk_") >= 0)
                {
                    CheckBox c = ctrl as CheckBox;
                    c.Checked = Read(sSection, ctrl.Name.Substring(5), c.Checked);
                }
                else if (ctrl.GetType() == typeof(ValueEdit) && ctrl.Name.IndexOf("edt_") >= 0)
                {
                    ValueEdit c = ctrl as ValueEdit;
                    c.Text = Read(sSection, ctrl.Name.Substring(4), c.Text);
                }
            }
        }

        public void AssignFromControls(string sSection, Control ctrlParent)
        {
            foreach (Control ctrl in ctrlParent.Controls)
            {
                if (ctrl.GetType() == typeof(ComboBox) && ctrl.Name.IndexOf("cb_") == 0)
                {
                    ComboBox c = ctrl as ComboBox;
                    Write(sSection, ctrl.Name.Substring(3), c.SelectedIndex);
                }
                else if (ctrl.GetType() == typeof(StateButton) && ctrl.Name.IndexOf("sbtn_") >= 0)
                {
                    StateButton c = ctrl as StateButton;
                    Write(sSection, ctrl.Name.Substring(5), c.State);
                }
                else if (ctrl.GetType() == typeof(CheckBox) && ctrl.Name.IndexOf("chk_") >= 0)
                {
                    CheckBox c = ctrl as CheckBox;
                    Write(sSection, ctrl.Name.Substring(5), c.Checked);
                }
                else if (ctrl.GetType() == typeof(ValueEdit) && ctrl.Name.IndexOf("edt_") >= 0)
                {
                    ValueEdit c = ctrl as ValueEdit;
                    Write(sSection, ctrl.Name.Substring(4), c.Text);
                }
            }
        }
    }
